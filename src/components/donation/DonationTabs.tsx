...
export type TabType = 'donasi' | 'kabar' | 'pencairan';
    ...
    return (
        <div className="sticky top-0 md:static z-30 bg-white dark:bg-card shadow-sm md:shadow-none border-b md:border-0 border-slate-100 dark:border-slate-800 md:rounded-2xl overflow-hidden">
            <div className="grid grid-cols-3 divide-x divide-slate-100 dark:divide-slate-800">
                ...
                <button
                    onClick={() => onTabChange('pencairan')}
                    className={cn("py-4 flex flex-col items-center gap-1 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors", activeTab === 'pencairan' ? "bg-purple-50/50 dark:bg-slate-800" : "")}
                >
                    <RefreshCcw className={cn("h-5 w-5", activeTab === 'pencairan' ? "text-purple-600" : "text-slate-400")} />
                    <span className={cn("text-xs font-bold", activeTab === 'pencairan' ? "text-purple-700" : "text-slate-500")}>Pencairan</span>
                </button>
            </div>
        </div>
    );
