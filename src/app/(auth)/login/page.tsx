...
export default function LoginPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (res.ok) {
                // Determine previous path logic here
                document.cookie = `mock_session_user=${JSON.stringify(data.user)}; path=/; max-age=86400`; // Simple client-side cookie/storage
    ...
    return (
        <div className="bg-white dark:bg-card p-8 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 space-y-6">
            <div className="text-center space-y-2">
                ...
                <p className="text-sm text-muted-foreground">Masuk untuk melanjutkan donasi Anda</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                >
                    {isLoading ? <Loader2 className="h-5 w-5 animate-spin" /> : <>Masuk <ArrowRight className="h-5 w-5" /></>}
                </button>
            </form>
